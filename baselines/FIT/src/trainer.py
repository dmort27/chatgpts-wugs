...
class BaseTrainer(object):
    ...
    def smart_load_model(self, model_prefix):
        ...
        self.models = [x[1] for x in sorted(models, key=lambda x: x[0])]
    ...
    def save_model(
        self, epoch_idx, devloss: float, eval_res: List[util.Eval], model_fp, seed
    ):
        eval_tag = "".join(["{}_{}.".format(e.desc, e.res) for e in eval_res])
        fp = f"{model_fp}_{seed}.nll_{devloss:.4f}.{eval_tag}epoch_{epoch_idx}"
    ...
    def run(self, start_epoch, decode_fn=None):
        ...
        for epoch_idx in range(start_epoch, max_epochs):
            ...
            if self.update_lr_and_stop_early(epoch_idx, devloss, params.estop):
                finish = True
                break
            self.save_model(epoch_idx, devloss, eval_res, params.model, params.seed)
