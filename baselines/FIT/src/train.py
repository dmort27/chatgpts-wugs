...
class Trainer(BaseTrainer):
    ...
    def decode(self, mode, batch_size, write_fp, decode_fn):
        ...
        cnt = 0
        sampler, nb_batch = self.iterate_batch(mode, batch_size)
        # TODO: output file name should be related to the language
        with open(f"{write_fp}_tur_{self.params.seed}.wug_{mode}.csv", "w") as fp:
            #fp.write("source\t prediction\t target\t loss\t dist\n")
            fp.write("prediction,origin\n")
            for src, src_mask, trg, trg_mask in tqdm(
                ...
                src = util.unpack_batch(src)
                for s, p, t, loss in zip(src, pred, trg, losses):
                    dist = util.edit_distance(p, t)
                    p = self.data.decode_target(p)
                    t = self.data.decode_target(t)
                    s = self.data.decode_source(s)
                    #fp.write(f'{"".join(s)}\t{"".join(p)}\t{"".join(t)}\t{loss.item()}\t{dist}\n')
                    fp.write(f'{"".join(p)},{"".join(t)}\n')
                    cnt += 1
...
def main():
    ...
    if params.training:
        ...
        trainer.smart_load_model(params.model + '_' + str(params.seed))
        trainer.logger.info("start testing")
        trainer.setup_training()
        results = trainer.decode('test', params.bs, 'test_res', decode_fn)
        print('Accuracy: {}'.format(results[0].res))
        print('Edit Distance: {}'.format(results[1].res))
